import torch
import os
from model import JointBert, print_activations_hook
...
def main(train_loader, dev_loader, _, lang, model, optimizer, criterion_slots, criterion_intents, clip=1, device='mps'):
...
if __name__ == '__main__':
    ...
    model.slot_out.register_forward_hook(print_activations_hook)
    model.intent_out.register_forward_hook(print_activations_hook)
    
    optimizer = optim.AdamW(model.parameters(), lr=lr)
    criterion_slots = nn.CrossEntropyLoss(ignore_index=tokenizer.pad_token_id)
    criterion_intents = nn.CrossEntropyLoss() # Because we do not have the pad token

    main(train_loader, dev_loader, test_loader, lang, model, optimizer, criterion_slots, criterion_intents, clip, device)